...
ffmpeg uses the raw camera feed. The h264 codec support on camera matters in order to provide low latency,
as in this case ffmpeg does no reencoding which significantly lowers latency.
...
to do so just start the container as such:
```bash
sudo docker run --rm -it \
